...
class Client:
    ...
    def sign_transaction(self, transaction_dict):
        """
        Signs the given transaction dictionary
        """
...
class Transaction:
    ...
    def execute_transaction(self, all_clients):
        """
        Executes transaction and updates balances
        """
        if isinstance(self.sender, str) and self.sender == "Genesis":
        ...
        if self.sender.balance >= self.value:
            ...
            if receiver_client:
                ...
                print("Warning: Receiver not found.")
                return False
        else:
            print(
                f"Transaction failed: {self.sender.identity[:10]}... "
                f"has insufficient funds ({self.sender.balance} < {self.value})"
            )
            return False
...
# Charlie → Alice (insufficient funds)
tx3 = Transaction(charlie, alice.identity, 100)
tx3.sign()

print("\nTransaction 3 (Charlie → Alice, 100 - Insufficient Funds):")
